...
export default function Home() {
  ...
  const menuList = ["XUẤT KHO", "CHI CHỢ", "TIỀN ĂN", "NHẬP PHIẾU"];
  const imageList = ["phieuxuat.png", "chicho.png", "tienan.png", "nhapphieu.png"];
  ...
  const handleClickMenu = (index) => {
    const pathMap = ["/phieuxuat", "/chicho", "/tienan", "/nhapphieu"];
        ...
        {progress > 0 && progress < 100 && (
          <Box sx={{ px: 2, mb: 2 }}>
            <LinearProgress
              variant="determinate"
              value={progress}
              sx={{ width: '20%', mx: 'auto' }} // 👈 Giảm chiều rộng tại đây
            />
            <Typography
              variant="caption"
              align="center"
              ...
              sx={{ mt: 1 }}
            >
              Đang xử lý dữ liệu... ({currentIndex} ngày - {progress}%)
            </Typography>
          </Box>
        )}
      </Box>
